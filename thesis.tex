...
% Glossary/notation list (abbreviations + custom entries)
% Requires running: makeglossaries <jobname> 
\usepackage[automake, abbreviations, sort=standard, nogroupskip, nopostdot, nohypertypes={abbreviations,main}, nonumberlist]{glossaries-extra}
\makeglossaries
\input{glossary}
...
\title{Engineering Protection in Multi-Mode Superconducting Circuits}
...
\titlePageTitle{
 Engineering Protection in \\ Multi-Mode Superconducting Circuits
}

% Who even are you ?
\author{Eli J.} % {First Middle}
  {Weissler} % {Last}

% How long have you already been in school?
\otherdegrees{
  B.S., Phsyics, Harvey Mudd College, 2019 \\
  M.S., Electrical Engineering, University of Colorado Boulder, 2024
}

% What degree is this thesis gonna get you?
\degree{Doctor of Philosophy} % {Long description}
  {Ph.D., Electrical Engineering}  % {Short description}

% What department?
\dept{Department of} % {Designation}
  {Electrical, Computer, and Energy Engineering} % {Name}

% The chair of your thesis committee.

\advisor{Prof.} % {Title}
  {Josh Combes}  % {Name}
\reader{Andr\'as Gyenis, Co-Chair}
% Other members of the thesis committee.
% \reader{Isaac Newton}
\readerThree{Scott Glancy}
\readerFour{Florent Lecocq}
% Uncomment if you have more committee members.
\readerFive{Marco Nicotra}
\readerSix{Daniel Slichter}
...
\abstract{ \OnePageChapter  % because it is very short
  This is my single-page abstract!
...
% include them here.
\input chapter-0-intro-background/chapter-0.tex
\input chapter-1-quantization/chapter-1.tex
\input chapter-2-modeling/chapter-2.tex
\input chapter-3-enumeration/chapter-3.tex
\input chapter-4-zero-pi/chapter-4.tex
\input chapter-5-rotor-codes/chapter-5.tex


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%            Glossary             %%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
{\let\clearpage\relax\let\cleardoublepage\relax\let\clearforchapter\relax
% \glsaddall % uncomment to list all defined terms
\printglossary[type=abbreviations, title=Glossary]}
